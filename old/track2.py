...
def draw_boxes(img, names, bbox, identities=None,offset=(0, 0)):
    ...
    #     label = '{:s}{:d}'.format(names[i], 1)
    #     t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 2, 2)[0]
        
    #     cv2.putText(img, label, (25, 25+i*25), cv2.FONT_HERSHEY_PLAIN, 2, [255, 255, 255], 2)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    ...
                    print(outputs)
                    print(identities)
                    draw_boxes(im0, names, bbox_xyxy, identities)
                    # to MOT format
